...
///////////////////////////////////////
#ifdef I2C_Extender_EN
  #define channelBtn  0
  #define channelR    1
  #define channelG    2
  #define channelB    3
  PCF8574 I2C_Extender(PCF8574_GPIO_ADDRESS);
#endif
///////////////////////////////////////
...
void OnboardDisplay::Update(int8_t updmode)
{
  ...
  if (((curms-m_LastUpdateMs) >= 1000) || (updmode == OBD_UPD_FORCE)) {
	...
	SetRedLed_EX(0);                   // restore the normal TEAL backlight
#ifdef LCD16X2 //Adafruit RGB LCD
	LcdSetBacklightColor(TEAL);
#endif
      }
...
void Btn::read()
{
  ...
  #ifdef I2C_Extender_EN
    sample = I2C_Extender.digitalRead(channelBtn) ;
...
void setup() {
  ...
  Serial.println("OpenEVSE_MOD_v0.1") ;
...
#ifdef I2C_Extender_EN
  I2C_Extender.pinMode(channelBtn, INPUT_PULLUP);
	I2C_Extender.pinMode(channelR, OUTPUT,LOW);
  I2C_Extender.pinMode(channelG, OUTPUT,LOW);
  I2C_Extender.pinMode(channelB, OUTPUT,LOW);
	I2C_Extender.begin();
#endif
  WDT_ENABLE();
}  
...
void SetRedLed_EX(uint8_t state) {
  if(state == 1) I2C_Extender.digitalWrite(channelR,LOW) ;
  else I2C_Extender.digitalWrite(channelR,HIGH) ;
}
void SetGreenLed_EX(uint8_t state) {
  if(state == 1) I2C_Extender.digitalWrite(channelG,LOW) ;
  else I2C_Extender.digitalWrite(channelG,HIGH) ;
}
void SetBlueLed_EX(uint8_t state) {
  if(state == 1) I2C_Extender.digitalWrite(channelB,LOW) ;
  else I2C_Extender.digitalWrite(channelB,HIGH) ;
}
